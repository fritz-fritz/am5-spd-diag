...
- Cut a release with `make bump TO=X.Y.Z MSG='...'`, merge to the default branch, then `git tag vX.Y.Z && git push origin vX.Y.Z`. PR CI runs `make test` on ubuntu-latest and `osc build` (chroot, no `osc commit`) for every `OSC_REPOS` target. GitHub branch protection cannot require a matrix job as one name; require the `ci` aggregator so a packaging failure blocks merge. Tags on a PR branch do not publish or commit to OBS. GitHub Actions uploads Source0, the vendor archive, and pinned rustc to OBS and attaches the packages to the GitHub Release.
